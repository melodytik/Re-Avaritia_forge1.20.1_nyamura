...
public class NoConsumeCatalystShapedRecipe extends ShapedTableCraftingRecipe {
    ...
    public @NotNull NonNullList<ItemStack> getRemainingItems(@NotNull IItemHandler inventory) {
        // 注意：ModCraftResultSlot.onTake() 的逻辑：
        //   1. 先对每个非空格子执行 removeItem(i, 1) —— 催化剂格子自然减少1个
        //   2. 如果 remainingItems 中对应格子非空，还会 grow(剩余数量) 再次累加
        // 因此，此处必须对催化剂格子返回 EMPTY，否则 onTake() 会把数量加回去导致越合成越多。
        // 催化剂的"消耗1个"完全由 onTake() 的 removeItem(i, 1) 完成。
